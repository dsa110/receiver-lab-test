# -*- coding: utf-8 -*-
"""
Created on Fri June 26 21:14 2020
...
datafilename = str('BEB_Test_Score_Sheet')
...
# TEMPLATE for Sandy
ws1['A1'] = 'Test Data for BEB Score Sheet'
...
ws1['G2'] = 'Thot, K'
ws1['H2'] = 'FEB/BEB Tn, K'
ws1['I2'] = 'Increase in Tsys due ot FEB/BEB'
...
ws1['Q2'] = 'BEB mA'
ws1['R2'] = 'LO  DC'
ws1['S2'] = 'BEB dBm mv  OFF'
ws1['T2'] = 'BEB dBm mv  ON'
...
print (' Please keep the same FEB during BEB tests')
Tester= input("Who is testing? Inititals:")
...
while(True):
    ...
    LgPwr_on = []
    lgpwr_on = []
    time.sleep(10) #comment this line if using 10kHz video BW
    spectrum_analyzer.write('*WAI')
    time.sleep(0.5)
    ...
    try:
        ...
        BEB_temp=(2000*a14_avg_value_NGENoff-500)/10
        
        if ((BEB_SN[len(BEB_SN)-1])==('A')):
           BEB_PD_mA=2000*a15_avg_value_NGENoff/1000 
           BEB_mA=2000*a13_avg_value_NGENoff/10
        ...
        if ((BEB_SN[len(BEB_SN)-1])==('B')):
           ...
           BEB_mA=2000*a9_avg_value_NGENoff/10
           BEB_IF_MON_NGENoff=2000*a8_avg_value_NGENoff
           BEB_IF_MON_NGENon=2000*a8_avg_value_NGENon
        
        BEB_LO_DC=2000*a10_avg_value_NGENoff/1000

    except:
 
        print ("Cannot turn noise gen on or off. Please check 28V or noise gen connectivity")
        print ("BEB Score Sheet data is saved")
        wb.save(str(folder+filename+'.xlsx'))
    
#    ws1['A'+str(row)] = BEB_SN
#    ws1['B'+str(row)] = FEB_SN
#    ws1['C'+str(row)] = "{:4.2f}".format(SA_data_NGENoff)
#    ws1['D'+str(row)] = "{:4.2f}".format(SA_data_NGENon)
#    ws1['E'+str(row)] = "{:4.2f}".format(Y_dB)
#    ws1['F'+str(row)] = "{:4.2f}".format(Y_ratio)
#    ws1['G'+str(row)] = "{:4.2f}".format(Tn)
#    ws1['H'+str(row)] = "{:4.2f}".format(Tn/3162)
#    ws1['I'+str(row)] = "{:4.2f}".format(BEB_out_dBm_per_MHz)
#    ws1['J'+str(row)] = "{:4.2f}".format(BEB_out_300k_totdBm)
#    ws1['K'+str(row)] = "{:4.2f}".format(BEB_out_26k_totdBm)
#    ws1['L'+str(row)] = str(t)
#    ws1['M'+str(row)] = Tester
#    ws1['N'+str(row)] = "{:4.2f}".format(BEB_temp) # BEB channel A
#    ws1['O'+str(row)] = "{:4.2f}".format(BEB_PD_mA) # BEB channel A
#    ws1['P'+str(row)] = "{:4.2f}".format(BEB_mA) # BEB channel A
#    ws1['Q'+str(row)] = "{:4.2f}".format(BEB_LO_DC) # BEB channel B
#    ws1['R'+str(row)] = "{:4.2f}".format(BEB_IF_MON_NGENoff) # BEB channel A
#    ws1['S'+str(row)] = "{:4.2f}".format(BEB_IF_MON_NGENon) # BEB channel A
    ws1['A'+str(row)] = BEB_SN
    ws1['B'+str(row)] = FEB_SN
    ...
    ws1['O'+str(row)] = float(BEB_temp) # BEB channel A
    ws1['P'+str(row)] = float(BEB_PD_mA) # BEB channel A
    ws1['Q'+str(row)] = float(BEB_mA) # BEB channel A
    ws1['R'+str(row)] = float(BEB_LO_DC) # BEB channel B
    ws1['S'+str(row)] = float(BEB_IF_MON_NGENoff) # BEB channel A
    ws1['T'+str(row)] = float(BEB_IF_MON_NGENon) # BEB channel A
    
    FEB_BEB_NF_dB=10*math.log10(1+Tn/290)
    print (" NF is : ",FEB_BEB_NF_dB,"dB")
    
    print ("Change BEB now if you want to continue!!")
...
#         except:
#             pass
#     adjusted_width = (max_length+0.5) * 1
